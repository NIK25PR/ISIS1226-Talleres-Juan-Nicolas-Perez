...
public class SandboxListas
{
    ...
    {
        HashMap<Integer, Integer> conteos = new HashMap<Integer, Integer>();
        for (int elemento : listaEnteros)
        {
            if (conteos.containsKey(elemento))
            {
                conteos.put(elemento, conteos.get(elemento) + 1);
            }
            else
            {
                conteos.put(elemento, 1);
            }
        }

        int contador = 0;
        for (Integer valor : conteos.keySet())
        {
            if (conteos.get(valor) > 1)
            {
                contador++;
            }
        }
